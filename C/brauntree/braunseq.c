/* standard library, defining NULL, malloc, free */
#include <stdlib.h>
...
/* Given a brauntree, swap it's subtrees. */
void swap(Tree tree);
/* balances a tree recursively */
...
{
  /* Basecase. 
     We have a brauntree with either 1 (left) child, or two NULL childs.
     Both should be NULL, since the left child has been added correctly in the recursion.
  */
  if ((*tree)->right == NULL) { 
    (*tree)->left = NULL;
    return;
  ...
  /* Recursive case.
     Make the left subtree of the roots right child, the subtree of the roots left child. 
     Recurse on roots right subtree.
  */
